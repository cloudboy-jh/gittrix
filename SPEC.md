...
Agent receives AgentSession (no promote method).
User UI receives UserSession (full control).
4. Concepts
4.1 Session
A session is one unit of agent work. Lifecycle:

Consumer calls gittrix.startSession({ task, durable, ephemeral })
Gittrix records the durable HEAD as baseline, creates an ephemeral fork of durable
Agent receives an AgentSession handle and works freely (read, write, commit, branch in ephemeral)
Consumer (human-facing UI) holds a UserSession and observes
User reviews diff against baseline, accepts a subset of changes
Consumer calls userSession.promote(...) — gittrix moves accepted changes to durable
Session evicts (immediately on promote, or via TTL if abandoned)
...
Promote strategies
Decided per-repo on first session, with auto-detection:
Detection (one-time):
...
4.6 Eviction
Sessions die. Default policy: ttl_idle: 4h + until_promote: true. Sessions can be configured per-session:
...
tsimport { GitTrix } from '@gittrix/core'
...
tsclass GittrixError extends Error { code: string }
...
ts// Provider callback (real interface)
new GitHubAdapter({
